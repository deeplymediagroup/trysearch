...
/**
 * Per-competitor keyword footprint (Workstream E2): the terms THIS competitor's listing is
 * optimized for, including its brand queries and derivatives ("<brand> alternative",
 * "apps like <brand>"). One small call per competitor; every candidate still passes the
 * live-autocomplete gate before it earns a row.
 *
 * @returns {Promise<string[]>}
 */
export async function generateCompetitorKeywords({ competitor, ownAppName = "", max = 30 }) {
  const result = await aiJson({
    system:
      "You are an ASO analyst reverse-engineering ONE competitor app's keyword strategy from its listing. Return the search phrases it is optimized for — lowercase, 1-4 words, no punctuation. Include its core feature and intent terms, its brand name as users type it, and brand derivatives ('<brand> alternative', 'apps like <brand>', '<brand> free'). Only phrases a real user would type.",
    prompt:
      `COMPETITOR\nName: ${competitor.name ?? ""}\nSubtitle: ${competitor.subtitle ?? ""}\nDescription: ${String(competitor.description ?? "").slice(0, 1500)}\n\n` +
      (ownAppName ? `MY APP (context only — do not generate its brand terms): ${ownAppName}\n\n` : "") +
      `Propose up to ${max} phrases.`,
    schema: {
      type: "object",
      properties: { candidates: { type: "array", items: { type: "string" } } },
      required: ["candidates"],
