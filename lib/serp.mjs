...
export async function liveKeywordMetrics(query, { term, platform, country }) {
  ...
  // Same calibration the nightly metrics job applies, for the same reason: the raw proxy sits
  // ~40 points above Apple's Search Popularity scale, and an on-demand lookup must not be the
  // one place in the app that still reports the inflated number.
  ...
  const est = applyProxyCalibration(pop.value, fit);
  const effective = popularityEffective({ popularity: null, popularity_estimate: est });
  ...
  await query(
    `update keywords
      ...
      where id = $1`,
    [kw.keyword_id, est, estDownloadsAtRank1({ popularity: effective, platform }), pop.value],
  );
