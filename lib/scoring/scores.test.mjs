...
describe("difficulty", () => {
  ...
  it("scores a SERP of huge incumbents as hard", () => {
    ...
    // `leaders` is the strength of the apps that actually TARGET the term (two ~40k title
    // matches here), shrunk toward the discounted field because only two of ten match.
    expect(d.parts.leaders).toBeGreaterThan(55);
    expect(d.parts.leadersBasis).toBe("title-matching incumbents");
  });

  it("does not treat a phrase nobody targets as defended", () => {
    // Same giant incumbents, but not one of them has the query in its name: the SERP is loose
    // matches, so the slot is a metadata problem, not a size problem.
    const defended = difficulty({ top: serpTop10, term: "meditation" });
    const loose = difficulty({ top: serpTop10, term: "quiet morning wind down ritual" });
    expect(loose.value).toBeLessThan(defended.value);
    expect(loose.parts.leadersBasis).toContain("nobody targets this term");
    expect(loose.value).toBeLessThan(30); // "low competition", which is the truth here
  });
