...
describe("difficulty", () => {
  ...
  it("treats a reported rating_count of 0 as missing, not as zero social proof", () => {
    // The long-tail failure mode measured against Sonar: the store reports no ratings for most
    // of a niche SERP, and averaging those zeros in scored a contested phrase as wide open.
    const withZeros = [
      { name: "ReciMe", rating_count: 269_947 },
      { name: "RecipeBox", rating_count: 13_881 },
      ...Array.from({ length: 8 }, (_, i) => ({ name: `Tiny App ${i}`, rating_count: 0 })),
    ];
    const d = difficulty({ top: withZeros, term: "save tiktok recipes" });
    expect(d.parts.medianStrength).toBe(141_914); // median of the two that DID report
    expect(d.parts.ratingsReportedBy).toBe(2);
    expect(d.value).toBeGreaterThan(45);
  });

  it("counts intent saturation, so purpose-built rivals are not read as easy", () => {
    const tiny = (name) => ({ name, rating_count: 0 });
    const saturated = difficulty({
      top: [tiny("Sift: Save Video Recipes"), tiny("Preplo: Recipes from Videos"), tiny("Clipzi Video Recipe Saver")],
      term: "save video recipes",
    });
    const unrelated = difficulty({
      top: [tiny("Chess Clock"), tiny("Bus Times"), tiny("Torch")],
      term: "save video recipes",
    });
    expect(saturated.parts.saturation).toBeGreaterThan(unrelated.parts.saturation);
    expect(saturated.value).toBeGreaterThan(unrelated.value);
  });
...
describe("popularity", () => {
  it("[§13] keeps the store's floored 5 and our estimate separately, capped in the censored band", () => {
    const kw = { popularity: 5, popularity_source: "store", popularity_estimate: 28 };
    // Both are retained. At the floor our proxy breaks the tie, but a term Apple censored is
    // by definition not in demand, so it cannot climb out of the low band.
    expect(kw.popularity).toBe(5);
    expect(kw.popularity_estimate).toBe(28);
    expect(popularityEffective(kw)).toBe(24);
  });

  it("never lets the proxy overrule a real Apple popularity above the floor", () => {
    // The bug this replaced: Apple said 45 for "motivation", the proxy said 87, we showed 87.
    expect(popularityEffective({ popularity: 45, popularity_estimate: 87 })).toBe(45);
    expect(popularityEffective({ popularity: 25, popularity_estimate: 94 })).toBe(25);
  });

  it("calibrates the proxy onto Apple's scale, and stays identity without enough pairs", () => {
    const pairs = [
      { proxy: 87, store: 45 },
      { proxy: 71, store: 25 },
      { proxy: 77, store: 25 },
    ...
    const fit = fitProxyCalibration(pairs);
    expect(fit.fitted).toBe(true);
    // A proxy at the top of the observed range must not land above Apple's observed maximum.
    expect(applyProxyCalibration(94, fit)).toBeLessThan(70);
    expect(applyProxyCalibration(94, fit)).toBeGreaterThanOrEqual(5);
    const thin = fitProxyCalibration([{ proxy: 80, store: 40 }]);
    expect(thin.fitted).toBe(false);
    expect(applyProxyCalibration(80, thin)).toBe(80);
  });
...
describe("revenue estimate", () => {
  ...
  it("keeps the off-chart installs x ARPU fallback at low confidence on both stores", () => {
    ...
    expect(android.method).toBe("rpi_benchmark");
  ...
  it("prices off-chart apps from measured rating velocity and the RevenueCat RPI benchmark", () => {
    const base = { platform: "ios", category: "Health & Fitness", iaps: [{ is_subscription: true, annualised_cents: 5999 }] };
    // 12,000 new ratings a month / 1.2% = 1M installs, x $0.66 D60 RPI halved = $330K/mo.
    const velocity = revenueEstimate({ ...base, monthlyRatings: 12_000 });
    expect(velocity.method).toBe("rpi_benchmark");
    expect(velocity.monthly_usd).toBe(330_000);
    expect(velocity.factors.join(" ")).toContain("MEASURED rating velocity");

    // A category with weaker economics must earn less on identical installs.
    const gaming = revenueEstimate({ ...base, category: "Games", monthlyRatings: 12_000 });
    expect(gaming.monthly_usd).toBeLessThan(velocity.monthly_usd);

    // Velocity beats the lifetime average when both are available.
    const lifetime = revenueEstimate({ ...base, ratingCount: 600_000, lifetimeMonths: 36 });
    expect(lifetime.factors.join(" ")).toContain("whole life");
  });
