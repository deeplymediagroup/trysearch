 ...
 */
import { median, roundToSigFigs, titleMatch, wordCount, words, tokens, round2 } from "./text.mjs";
...
/**
 * APPLE'S OWN NUMBER WINS. This used to prefer our proxy, which is why our popularity read
 * far higher than every other tool: Apple says "motivation" is 45 and our uncalibrated
 * autocomplete proxy said 87. The proxy exists for ONE job — telling apart the long-tail terms
 * Apple censors by flooring them at 5 (§1.4) — and it must never overrule a real measurement.
 *
 * So: a real store value above the floor is the answer. At the floor (5), the proxy breaks the
 * tie between all the terms Apple lumped together, but can never claim more than the floor
 * itself plus the censored band it lives in.
 */
export const SAP_FLOOR = 5;

export function popularityEffective(keyword) {
  const store = keyword?.popularity ?? null;
  const proxy = keyword?.popularity_estimate ?? null;
  if (store != null && store > SAP_FLOOR) return store;
  if (store === SAP_FLOOR && proxy != null) {
    // Censored band: keep it visibly low. A floored term is by definition not in demand.
    return Math.min(proxy, 24);
  }
  return store ?? proxy;
}

/**
 * Least-squares fit of our autocomplete proxy onto Apple's Search Popularity scale, from the
 * keywords where we happen to know BOTH. The proxy is a rank-ish signal with no natural units,
 * so without this it free-runs 40 points high; with even a handful of paired observations it
 * lands in Apple's range.
 *
 * Returns an identity fit under 4 pairs — an under-determined fit is worse than none.
 *
 * @param {{proxy: number, store: number}[]} pairs
 */
export function fitProxyCalibration(pairs = []) {
  const clean = pairs.filter((p) => typeof p?.proxy === "number" && typeof p?.store === "number" && p.store > SAP_FLOOR);
  if (clean.length < 4) return { slope: 1, intercept: 0, n: clean.length, fitted: false };

  const mx = clean.reduce((s2, p) => s2 + p.proxy, 0) / clean.length;
  const my = clean.reduce((s2, p) => s2 + p.store, 0) / clean.length;
  const varX = clean.reduce((s2, p) => s2 + (p.proxy - mx) ** 2, 0);
  const cov = clean.reduce((s2, p) => s2 + (p.proxy - mx) * (p.store - my), 0);
  // No spread in the proxy → a slope is meaningless; shift to the observed mean instead.
  const slope = varX === 0 ? 0 : cov / varX;
  return { slope: round2(slope), intercept: round2(my - slope * mx), n: clean.length, fitted: true };
}

/** Applies a fit from fitProxyCalibration, clamped to Apple's own 5-100 scale. */
export function applyProxyCalibration(proxy, fit) {
  if (proxy == null) return null;
  if (!fit?.fitted) return proxy;
  return Math.max(SAP_FLOOR, Math.min(100, Math.round(fit.slope * proxy + fit.intercept)));
}
...
export function difficulty({ top, term, serpDepth = null, platform = "ios" } = {}) {
  ...
  if (!strengths.length) return { value: null, parts: null };

  // A rating_count of 0 means "the store reported no ratings for this app in this storefront",
  // and half a long-tail SERP comes back that way. Feeding those zeros into the median dragged
  // it to 0 and scored genuinely contested phrases as wide open: measured against Sonar on 17
  // shared US keywords, "save tiktok recipes" came out 6 where Sonar says 65. Zeros are missing
  // data, so the median is taken over the apps that actually reported ratings.
  const reported = strengths.filter((n) => n > 0);
  const med = median(reported.length ? reported : strengths);
  ...
  // Intent saturation: the mean share of the query's words each top-10 app carries in its name.
  // This is what full title-matching misses — ten purpose-built rivals ("Sift: Save Video
  // Recipes", "Preplo: Recipes from Videos") can be tiny AND new, so strength reads them as
  // easy, while the phrase is in fact fully served. Saturation sees them.
  const queryWords = words(term);
  const saturation = queryWords.length
    ? (100 * top.reduce((sum, a) => {
        const bag = tokens(a.name);
        return sum + queryWords.filter((w) => bag.has(w)).length / queryWords.length;
      }, 0)) / top.length
    : 0;
  ...
  // Weights fitted against the reference tool's difficulty on 52 shared keyword-market pairs
  // (US/GB/DE/IT/FR/CA/CZ, floor terms through brand giants like youtube at 84). On that set
  // saturation and title match carry ZERO weight — incumbent strength plus a small phrase-length
  // discount explains it all (MAE 6.7 vs 11 for the previous 4-component mix). Both are still
  // computed and reported in parts because the breakdown popover explains scores with them.
  // The +16 is a floor: taking any slot from any incumbent costs something.
  const value = Math.max(0, Math.min(100, Math.round(0.6 * leaders + 0.05 * specificity + 16)));
  ...
  return {
    ...
    parts: {
      ...
      medianStrength: med,
      ratingsReportedBy: reported.length,
...
/**
 * Top-grossing chart position -> revenue. This is the primary signal now, and the reason the
 * old model was wrong: it derived revenue from rating counts, which for a subscription app
 * understates by an order of magnitude and cannot see a price change at all.
 *
 * Apple publishes a daily top-grossing chart per category and storefront, and grossing rank IS
 * revenue rank — the only public number that orders apps by money. Chart revenue follows a
 * power law in rank, so ONE calibrated point on a chart lets the whole chart be read:
 *
 *     monthly(rank) = anchorMonthly * (anchorRank / rank) ** GROSSING_EXPONENT
 *
 * GROSSING_EXPONENT ~0.85 is the decay of a grossing chart inside a single category: an
 * empirical shape, not an Apple constant, so it lives here as one named number to re-fit
 * against real data rather than buried inside an expression.
 *
 * No anchor -> NO DOLLAR FIGURE. An uncalibrated power law is a random number generator with a
 * plausible shape, which is exactly what the old estimator shipped.
 */
/**
 * Revenue per install at day 60, from RevenueCat's State of Subscription Apps 2026 (p.30-31):
 * medians across 115,000+ subscription apps. THIRD-PARTY MEASURED ECONOMICS — the first number
 * in this file that comes from real money across a large sample rather than a modelled chain.
 *
 * It replaces the old "2% convert x median IAP price x retention" guesswork, which multiplied
 * three unverifiable assumptions and could be off by an order of magnitude in either direction.
 *
 * Keys are lowercased Apple/Play primary genres. RPI_D60_DEFAULT is the all-category median.
 */
export const RPI_D60_DEFAULT = 0.34;

export const RPI_D60_BY_CATEGORY = {
  "health & fitness": 0.66,
  business: 0.50,
  education: 0.45,
  games: 0.14,
...
export function rpiForCategory(category) {
  if (!category) return RPI_D60_DEFAULT;
  return RPI_D60_BY_CATEGORY[String(category).toLowerCase()] ?? RPI_D60_DEFAULT;
}

/**
 * Steady-state monthly revenue from a steady install rate.
 *
 * RPI at D60 is CUMULATIVE revenue per install over that install's first 60 days, so in steady
 * state each month's revenue is roughly one month's worth of that curve: RPI_D60 / 2 per
 * install per month. It ignores revenue past day 60, so it reads LOW for a sticky annual-plan
 * app — which is the right direction for an estimate to be wrong in.
 */
export function revenueFromInstalls({ monthlyInstalls, rpiD60 = RPI_D60_DEFAULT } = {}) {
  if (!monthlyInstalls || monthlyInstalls <= 0) return null;
  return monthlyInstalls * (rpiD60 / 2);
}

export const GROSSING_EXPONENT = 0.85;

export function revenueFromGrossingRank({ rank, anchorRank, anchorMonthlyUsd } = {}) {
  if (!rank || rank < 1 || !anchorRank || anchorRank < 1 || !anchorMonthlyUsd) return null;
  return anchorMonthlyUsd * (anchorRank / rank) ** GROSSING_EXPONENT;
}

/**
 * Public data only, best signal first. Confidence must be reported and the floor honoured.
 *
 *   1. measuredMonthlyUsd    — real proceeds (App Store Connect / Play). Not an estimate at all.
 *   2. grossingRank + anchor — the power law above, anchored to a measured app on the SAME chart.
 *   3. grossingRank alone    — the rank is reported, dollars are not. Honest beats confident.
 *   4. installs x ARPU       — last resort, off-chart apps only, widest band, 'low' forever.
 */
export function revenueEstimate({
  platform = "ios",
  measuredMonthlyUsd = null,
  grossingRank = null,
  grossingChartLabel = null,
  anchor = null, // { rank, monthlyUsd, label }
  realInstalls = null,
  ratingCount = null,
  monthlyRatings = null, // MEASURED new ratings per month, from our own snapshot history
  ratingsPerInstall = 0.012, // ~1.2% of installers rate; category benchmark
  priceCents = 0,
  iaps = [],
  category = null, // primary genre, for the RevenueCat RPI benchmark
  lifetimeMonths = null,
} = {}) {
  const model = revenueModel({ priceCents, iaps });
  const out = (extra) => ({ model, ...extra });

  // 1. Measured. Never model over the truth.
  if (typeof measuredMonthlyUsd === "number" && measuredMonthlyUsd > 0) {
    return out({
      method: "measured",
      confidence: "high",
      grossing_rank: grossingRank ?? null,
      monthly_usd: Math.round(measuredMonthlyUsd),
      monthly_usd_low: Math.round(measuredMonthlyUsd),
      monthly_usd_high: Math.round(measuredMonthlyUsd),
      display: `${formatMoneyShort(measuredMonthlyUsd)}/mo`,
      factors: ["Real proceeds reported by the store, not an estimate. Approximate FX only."],
    });
  }

  // 2. Grossing rank, calibrated against a measured app on the same chart.
  if (grossingRank && anchor?.rank && anchor?.monthlyUsd) {
    const monthly = revenueFromGrossingRank({ rank: grossingRank, anchorRank: anchor.rank, anchorMonthlyUsd: anchor.monthlyUsd });
    if (monthly != null) {
      return out({
        method: "grossing_rank",
        confidence: "medium",
        grossing_rank: grossingRank,
        monthly_usd: Math.round(monthly),
        monthly_usd_low: Math.round(roundToSigFigs(monthly * 0.5, 2)),
        monthly_usd_high: Math.round(roundToSigFigs(monthly * 2, 2)),
        display: monthly < REVENUE_FLOOR_USD ? `<$${REVENUE_FLOOR_USD / 1000}K/mo` : `${formatMoneyShort(monthly)}/mo`,
        factors: [
          `Top-grossing rank #${grossingRank}${grossingChartLabel ? ` on ${grossingChartLabel}` : ""} — the only public number that ranks apps by money.`,
          `Calibrated against ${anchor.label ?? "a measured app"} at rank #${anchor.rank} earning ${formatMoneyShort(anchor.monthlyUsd)}/mo.`,
          `Power law: monthly = anchor x (anchorRank / rank) ^ ${GROSSING_EXPONENT}. Band is x0.5 to x2.`,
        ],
      });
    }
  }

  // 3. On a grossing chart but nothing to calibrate with. Report the rank, refuse the dollars.
  if (grossingRank) {
    return out({
      method: "rank_only",
      confidence: "low",
      grossing_rank: grossingRank,
      monthly_usd: null,
      monthly_usd_low: null,
      monthly_usd_high: null,
      display: `#${grossingRank} grossing`,
      factors: [
        `Top-grossing rank #${grossingRank}${grossingChartLabel ? ` on ${grossingChartLabel}` : ""}.`,
        "No calibration anchor yet: connect App Store Connect for an own app on this chart, or set REVENUE_ANCHOR_USD_MONTH, and every app on the chart gets a dollar figure.",
      ],
    });
  }

  // 4. Off-chart fallback: installs x ARPU. Weak by construction, and labelled as such.
  let monthlyInstalls = null;
  const factors = [];

  // Rating VELOCITY first. A lifetime rating total spread over the app's whole life answers
  // "how big has this app ever been", which is the wrong question — a five-year-old app that
  // has stopped growing looks identical to one acquiring hard today. New ratings per month is
  // something we measure ourselves from consecutive snapshots.
  if (typeof monthlyRatings === "number" && monthlyRatings > 0) {
    monthlyInstalls = monthlyRatings / ratingsPerInstall;
    factors.push(
      `Not on a top-grossing chart. Installs from MEASURED rating velocity — ${Math.round(monthlyRatings).toLocaleString()} ` +
        `new ratings a month across our own snapshots — at an assumed ${(ratingsPerInstall * 100).toFixed(1)}% rating rate.`,
    );
  } else if (platform === "android" && typeof realInstalls === "number" && realInstalls > 0) {
    const months = Math.max(lifetimeMonths ?? 24, 1);
    monthlyInstalls = realInstalls / months;
    factors.push(`Not on a top-grossing chart. Play publishes an exact lifetime install count (${realInstalls.toLocaleString()}), spread over ~${months} months.`);
  } else if (typeof ratingCount === "number" && ratingCount > 0) {
    const months = Math.max(lifetimeMonths ?? 24, 1);
    monthlyInstalls = ratingCount / ratingsPerInstall / months;
    factors.push(
      `Not on a top-grossing chart, and we have no rating history yet, so installs are averaged over the app's ` +
        `whole life from ${ratingCount.toLocaleString()} ratings at an assumed ${(ratingsPerInstall * 100).toFixed(1)}% ` +
        `rating rate. Weakest input on the page — an order of magnitude at best.`,
    );
  }
  ...
  if (monthlyInstalls == null) {
    return out({
      ...
      monthly_usd_high: null,
      display: "—",
      factors: ["No chart, install or rating signal available."],
    });
  }

  // Monetisation now comes from a measured third-party benchmark, not a chain of assumptions.
  const rpi = rpiForCategory(category);
  const monthlyUsd = model === "paid"
    ? (monthlyInstalls * priceCents) / 100
    : revenueFromInstalls({ monthlyInstalls, rpiD60: rpi });

  if (model === "paid") {
    factors.push(`Paid app at ${(priceCents / 100).toFixed(2)} per download.`);
  } else {
    factors.push(
      `Monetisation from RevenueCat's State of Subscription Apps 2026 median revenue per install ` +
        `at day 60 — $${rpi.toFixed(2)} for ${category ? String(category).toLowerCase() : "all categories"}, ` +
        `across 115,000+ apps — halved to a steady-state monthly rate.`,
    );
    if (iaps.length) factors.push(`${iaps.length} real in-app price(s) scraped, used for the model classification.`);
  }

  return out({
    method: "rpi_benchmark",
    confidence: "low",
    grossing_rank: null,
    monthly_usd: Math.round(monthlyUsd),
    // Wide by design: the install count underneath it is modelled on iOS.
    monthly_usd_low: Math.round(roundToSigFigs(monthlyUsd * 0.4, 2) ?? 0),
    monthly_usd_high: Math.round(roundToSigFigs(monthlyUsd * 2.5, 2) ?? 0),
