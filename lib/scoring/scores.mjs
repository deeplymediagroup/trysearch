 ...
 */
import { median, roundToSigFigs, titleMatch, wordCount, round2 } from "./text.mjs";
...
export function difficulty({ top, term, serpDepth = null, platform = "ios" } = {}) {
  ...
  const matching = top.filter((a) => titleMatch(a.name, term));
  ...
  // Whose strength actually counts. The old version took the median of WHOEVER Apple returned,
  // which made every long-tail phrase look defended: Apple fills a thin SERP with big loosely
  // matched apps that are not targeting the phrase at all, so a 4-word query inherited the
  // rating counts of category giants and scored ~46 — "moderate" — when it was wide open.
  //
  // Strength now comes from the apps that DO target the term, and when nobody targets it the
  // incumbents' size is discounted, because out-ranking an app that is not defending a phrase
  // is a metadata problem, not a size problem.
  const relevantStrengths = matching.map((a) => strengthOf(a, platform)).filter((n) => n != null);
  const relevantMed = median(relevantStrengths);
  // 1k ratings ≈ 50, 100k ≈ 83, 1M = 100.
  const logStrength = (n) => Math.min(100, (100 * Math.log10(n + 1)) / 6);
  const UNDEFENDED_DISCOUNT = 0.5;
  const undefended = logStrength(med) * UNDEFENDED_DISCOUNT;
  // Shrinkage: ONE title-matching giant is not proof a phrase is defended, it is a sample of
  // one. Trust the relevant median in proportion to how many apps actually target the term,
  // and fall back toward the discounted field for the rest.
  const w = Math.min(1, matches / 3);
  const leaders = relevantMed == null ? undefended : w * logStrength(relevantMed) + (1 - w) * undefended;
  ...
  return {
    ...
    parts: {
      ...
      leadersBasis: relevantMed != null ? "title-matching incumbents" : "all incumbents, discounted 50% (nobody targets this term)",
      medianRelevantStrength: relevantMed,
