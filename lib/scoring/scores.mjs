...
export function revenueEstimate({
  ...
  if (platform === "android" && typeof realInstalls === "number" && realInstalls > 0) {
    ...
    factors.push(
      `Not on a top-grossing chart, so installs are modelled from ${ratingCount.toLocaleString()} ratings at an assumed ` +
        `${(ratingsPerInstall * 100).toFixed(1)}% rating rate. Weak input — treat the number as an order of magnitude.`,
    );
  }
  ...
  if (model === "paid") {
    ...
    const medianAnnual = median(iaps.map((i) => i.annualised_cents).filter((n) => typeof n === "number" && n > 0));
    if (medianAnnual != null) {
      arpuCents = medianAnnual / 12;
  ...
  return out({
    method: "installs_arpu",
    confidence: "low",
    grossing_rank: null,
    monthly_usd: Math.round(monthlyUsd),
    // Wider than the old x0.6-x1.6: an uncalibrated install model does not deserve a tight band.
