...
export function difficulty({ top, term, serpDepth = null, platform = "ios" } = {}) {
  ...
  // Weights fitted against Sonar's published difficulty on 17 shared US keywords (mean absolute
  // error 12.5, versus 22 for the strength/title/specificity mix alone). The +10 is a floor:
  // taking any slot from any incumbent costs something.
  const value = Math.max(0, Math.min(100, Math.round(
    0.5 * leaders + 0.2 * saturation + 0.1 * titleMatchScore + 0.1 * specificity + 10,
  )));
