...
export async function impressionShare({ adamId, country, reportType = "FIRST_SLOT", pageSize = 5000 } = {}) {
  for (let weeksBack = 0; weeksBack <= 1; weeksBack++) {
    ...
    const res = await adsPost("/insights/apps/impression-share/query", {
      // IN with arrays, not EQUALS — the live API 400s on EQUALS here (verified 2026-08-22).
      filters: [
        { field: "promotedObjectId", operator: "IN", value: [String(adamId)] },
        { field: "countryOrRegion", operator: "IN", value: [String(country).toUpperCase()] },
      ],
