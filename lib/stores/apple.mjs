 ...
 */
export async function appleLookup(ids, country = "us", { english = false, cache = true } = {}) {
  ...
  for (let i = 0; i < unique.length; i += 200) {
    ...
    const { data } = await fetchJson(url, {
      cacheKey: cache ? `lookup:${country}:${english ? "en" : "native"}:${batch[0]}:${batch.length}` : undefined,
      ttl: TTL.appMetaIos,
    });
...
export async function appleInAppPurchases(appId, country = "us") {
  ...
  const pairs = [
    ...flat.matchAll(/\{"\$kind":"textPair","leadingText":"((?:[^"\\]|\\.)*)","trailingText":"\$([\d,]+\.\d\d)"\}/g),
  ];
  ...
  for (const m of pairs) {
    ...
    const priceCents = Math.round(Number(m[2].replace(/,/g, "")) * 100);
