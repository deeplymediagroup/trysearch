...
// ---------------------------------------------------------------------------
// Reviews — batchexecute rpcid oCPfdb (Workstream K)
// ---------------------------------------------------------------------------

/**
 * Index paths into an oCPfdb review row, vendored as CONFIGURATION per 02 §6.7: Google
 * rotates these without notice, so they live in one visible place and every parse is
 * validated before use. A shape change degrades this feature (empty array + a caller
 * warning), never the crawl.
 */
const REVIEW_PATHS = {
  id: [0],
  author: [1, 0],
...
const atPath = (row, path) => path.reduce((v, i) => (v == null ? v : v[i]), row);

/**
 * Newest reviews for a Play package. This is the batchexecute endpoint — "the most brittle
 * component in the entire spec" (02 §6.4) — so it is optional by design: callers treat a
 * thrown shape error as "skip Android reviews today", not as a crawl failure.
 *
 * Returns the same shape appleReviews feeds the reviews job:
 * { reviews: [{ store_review_id, rating, title, body, author, app_version, helpful_count, reviewed_at }] }
 */
export async function playReviews(pkg, country = "us", { count = 100, language = "en" } = {}) {
  const inner = JSON.stringify([null, [2, 2, [Math.min(count, 150), null, null], null, []], [String(pkg), 7]]);
  const freq = JSON.stringify([[["oCPfdb", inner, null, "generic"]]]);

  // Same stale-but-unvalidated 2019 session values playSuggest uses.
  const url =
    "https://play.google.com/_/PlayStoreUi/data/batchexecute" +
    "?rpcids=oCPfdb&f.sid=-697906427155521722&bl=boq_playuiserver_20190903.08_p0" +
    `&hl=${language}&gl=${country}&authuser&soc-app=121&soc-platform=1&soc-device=1&_reqid=1065213`;
  ...
  const { text } = await fetchText(url, {
    ...
    headers: { "Content-Type": "application/x-www-form-urlencoded;charset=UTF-8" },
    body: `f.req=${encodeURIComponent(freq)}`,
    cacheKey: `reviews:android:${country}:${pkg}`,
    ttl: TTL.reviews,
  });
  ...
  try {
    const input = JSON.parse(text.substring(5)); // strip the )]}' prefix
    const data = JSON.parse(input[0][2]); // JSON inside a string — parse twice
    rows = data?.[0] ?? [];
  } catch {
    throw new Error("oCPfdb response is not parseable — Google likely rotated the rpcid (02 §6.4).");
  }
  if (!Array.isArray(rows)) throw new Error("oCPfdb payload shape changed — expected an array of review rows.");

  const reviews = [];
  for (const row of rows) {
    const id = atPath(row, REVIEW_PATHS.id);
    const rating = Number(atPath(row, REVIEW_PATHS.rating));
    // Validate, never trust: a silently-shifted index path yields garbage that LOOKS full.
    if (!id || !(rating >= 1 && rating <= 5)) continue;
    const seconds = Number(atPath(row, REVIEW_PATHS.date_seconds));
    reviews.push({
      store_review_id: String(id),
      rating,
      title: null, // Play reviews have no titles
      body: atPath(row, REVIEW_PATHS.body) ?? null,
      author: atPath(row, REVIEW_PATHS.author) ?? null,
      app_version: atPath(row, REVIEW_PATHS.version) ?? null,
      helpful_count: Number(atPath(row, REVIEW_PATHS.helpful)) || null,
      reviewed_at: Number.isFinite(seconds) && seconds > 0 ? new Date(seconds * 1000).toISOString() : null,
    });
  }
  if (rows.length > 0 && reviews.length === 0) {
    throw new Error(`oCPfdb returned ${rows.length} rows but none parsed — index paths have moved.`);
  }
  return { reviews };
}
