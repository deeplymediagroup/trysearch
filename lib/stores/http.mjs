...
const BUDGETS = {
  ...
  // search.itunes.apple.com carries BOTH autocomplete and the ranked SERP, which is ~85% of
  // every crawl. It was measured at 40 calls in 21s (~114/min) with zero failures, and a
  // later 492-call run logged a 153ms average and zero throttles. At 40/min we were adding
  // ~1.4s of pure sleep to every call on the busiest host — 83% of total crawl wall-clock
  // was the limiter waiting, not the network. 80/min keeps a ~30% margin under the measured
  // ceiling while roughly halving the nightly run. Adaptive backoff still catches a 429.
  "apple-storefront": 80,
