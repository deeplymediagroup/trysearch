...
/**
 * TWO keys, because Apple's roles don't overlap: the Sales-role key (ASC_*) can read sales
 * and analytics reports but not App Store versions; the App Manager key (ASC_MANAGER_*) can
 * read the hidden keywords field but 403s on sales reports. Each helper picks its prefix.
 */
function ascPrivateKey(prefix = "ASC_") {
  // Vercel env vars can't hold a real file, so *_PRIVATE_KEY (raw PEM content) is the prod
  // path; *_PRIVATE_KEY_FILE (a local .p8 path) is the dev-machine convenience.
  if (process.env[`${prefix}PRIVATE_KEY`]) return process.env[`${prefix}PRIVATE_KEY`];
  const file = process.env[`${prefix}PRIVATE_KEY_FILE`];
  if (file && fs.existsSync(file)) return fs.readFileSync(file, "utf8");
  return null;
}

export function ascManagerConfigured() {
  return Boolean(process.env.ASC_MANAGER_KEY_ID && ascPrivateKey("ASC_MANAGER_"));
}

const b64url = (buf) => Buffer.from(buf).toString("base64").replace(/=+$/, "").replace(/\+/g, "-").replace(/\//g, "_");

const cached = new Map(); // prefix -> { token, expiresAt }
 ...
 */
function jwt(prefix = "ASC_") {
  const now = Math.floor(Date.now() / 1000);
  const hit = cached.get(prefix);
  if (hit && hit.expiresAt > now + 30) return hit.token;

  // The issuer is the TEAM id, shared by every key of the team — the manager pair only needs
  // its own key id + private key, and falls back to ASC_ISSUER_ID.
  const issuerId = process.env[`${prefix}ISSUER_ID`] ?? process.env.ASC_ISSUER_ID;
  const keyId = process.env[`${prefix}KEY_ID`];
  const privateKey = ascPrivateKey(prefix);
  ...
  cached.set(prefix, { token, expiresAt: exp });
  return token;
}

async function ascGet(pathOrUrl, { presigned = false, prefix = "ASC_" } = {}) {
  const url = pathOrUrl.startsWith("http") ? pathOrUrl : `${API}${pathOrUrl}`;
  // Segment URLs are pre-signed by Apple — sending an Authorization header on those can 400.
  const headers = presigned ? {} : { Authorization: `Bearer ${jwt(prefix)}` };
...
async function ascJson(path, opts = {}) {
  const res = await ascGet(path, opts);
...
/**
 * The hidden 100-char keywords field + promotional text, per locale. Only ASC can see the
 * keywords field — it is never public. Needs a key with App Manager (or Admin) role; a
 * Sales-and-Reports key gets 403 here, which callers must treat as "not available", not
 * an error (missing credentials degrade, never throw — house rule #6).
 *
 * @returns {Promise<Array<{locale: string, keywords: string|null, promotional_text: string|null}>|null>}
 */
export async function fetchListingLocalizations(appleAppId) {
  // Prefer the App Manager key — the Sales key 403s on App Store versions.
  const prefix = ascManagerConfigured() ? "ASC_MANAGER_" : "ASC_";
  if (prefix === "ASC_" && !ascConfigured()) return null;
  try {
    const versions = await ascJson(`/apps/${appleAppId}/appStoreVersions?filter[appStoreState]=READY_FOR_SALE&limit=1`, { prefix });
    const versionId = versions?.data?.[0]?.id;
    if (!versionId) return null;
    const locs = await ascJson(`/appStoreVersions/${versionId}/appStoreVersionLocalizations?limit=50`, { prefix });
    if (!locs?.data) return null;
    return locs.data.map((l) => ({
      locale: l.attributes?.locale ?? "en-US",
      keywords: l.attributes?.keywords ?? null,
      promotional_text: l.attributes?.promotionalText ?? null,
    }));
  } catch (err) {
    // 403 = the key's role can't read App Store versions. Degrade silently.
    if (String(err.message).includes("403")) return null;
    throw err;
  }
}
