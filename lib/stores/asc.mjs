...
export async function fetchListingLocalizations(appleAppId) {
  if (!ascConfigured()) return null;
  try {
    const versions = await ascJson(`/apps/${appleAppId}/appStoreVersions?filter[appStoreState]=READY_FOR_SALE&limit=1`);
    const versionId = versions?.data?.[0]?.id;
    if (!versionId) return null;
    const locs = await ascJson(`/appStoreVersions/${versionId}/appStoreVersionLocalizations?limit=50`);
