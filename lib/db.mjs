...
export async function connect() {
  ...
  // A Pool, not a single Client: crawl jobs sit through multi-minute AI calls, Neon closes
  // the idle connection, and a dead Client emits an unhandled 'error' event that kills the
  // whole process (bit the discovery job 2026-08-22). A pool replaces dead connections per
  // query. No .mjs code uses transactions, so per-query connections change nothing.
  const pool = new Pool({ connectionString: url, ssl: { rejectUnauthorized: false }, max: 3 });
  pool.on("error", () => {}); // an idle connection dying is routine, not fatal
  await pool.query("select 1"); // fail fast on a bad URL, like Client.connect() did
  return pool;
}
